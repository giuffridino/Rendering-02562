...
async function main()
{
    ...
    var obj_filename = "3D_Meshes/dumb_cube.obj";
    var g_objDoc = null;
    ...
    //     if (!response.ok) {
    //       throw new Error(`Failed to load OBJ file from ${filePath}`);
    //     }
    ...
    //     // Implement the parsing logic to extract vertex positions, normals, UVs, and indices
    //     // and organize them into appropriate data structures (e.g., arrays)
    // }
    ...
    //     usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    // });
    // var vPositions = [vec4(-1.000000, -1.000000, 1.000000, 0.0), vec4(-1.000000, 1.000000, 1.000000, 0.0), vec4(-1.000000, -1.000000, -1.000000, 0.0), vec4(-1.000000, -1.000000, -1.000000, 0.0)]
    // device.queue.writeBuffer(vPositionsBuffer, 0, new Float32Array(flatten(vPositions)))
    // const meshFacesBuffer = device.createBuffer({
    //     size: 64, // number of bytes
    //     usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
    // });
    // var meshFaces = [vec4(1, 2, 4, 3)];
